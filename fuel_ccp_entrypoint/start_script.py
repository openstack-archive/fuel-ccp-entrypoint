...
def check_tls_options():
    for s in ('etcd', 'rabbitmq', 'galera'):
        if VARIABLES.get(s) and VARIABLES[s]['tls']['enabled']:
            return True
    if ((VARIABLES['security']['tls'].get('openstack') and
         VARIABLES['security']['tls']['openstack']['enabled'])):
        return True
    return False
...
def run_probe(probe):
    if probe["type"] == "exec":
        ...
        # TODO(skraynev): move this to separate config options and remove
        #                 this check from the code.
        if VARIABLES['security']['tls'].get('openstack'):
            if VARIABLES['security']['tls']['openstack']['enabled']:
                scheme = 'https'
